    ...
    >
      <motion.div
        ...
        style={{
          ...
          boxShadow: "0 20px 60px rgba(0,0,0,0.6), 0 0 40px rgba(1,54,254,0.05)",
        }}
        initial={{ opacity: 0, scale: 0.9, y: 20 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        transition={{ type: "spring", stiffness: 300, damping: 25 }}
      >
        <h2 id="confirm-modal-title" className="text-lg font-bold text-[#0136fe] mb-2">
          {title}
        </h2>
        <p className="text-sm mb-6" style={{ color: "rgba(1,54,254,0.7)" }}>{message}</p>
        <div className="flex justify-end gap-3">
          ...
          <motion.button
            onClick={onConfirm}
            className="px-4 py-2 text-sm font-semibold text-[#0136fe] rounded-lg transition"
